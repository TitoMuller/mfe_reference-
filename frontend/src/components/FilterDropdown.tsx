  ...
  const getDisplayPills = () => {
    ...
    if (selectedValues.length <= 2) {
      return selectedValues.map(item => (
        <span 
          key={item}
          className="inline-flex items-center px-2 py-0.5 rounded text-xs bg-blue-600 text-white"
        >
          {item}
        </span>
      ));
    }
    // Show first items + count (like screenshot: "my-awes... robson-t... +3")
    return [
      <span 
        key={selectedValues[0]}
        className="inline-flex items-center px-2 py-0.5 rounded text-xs bg-blue-600 text-white"
      >
        {selectedValues[0].length > 8 ? `${selectedValues[0].substring(0, 8)}...` : selectedValues[0]}
      </span>,
      selectedValues.length > 1 && (
        <span 
          ...
          className="inline-flex items-center px-2 py-0.5 rounded text-xs bg-blue-600 text-white"
        >
          {selectedValues[1].length > 8 ? `${selectedValues[1].substring(0, 8)}...` : selectedValues[1]}
        </span>
      ),
      selectedValues.length > 2 && (
        <span 
          key="more"
          className="inline-flex items-center px-2 py-0.5 rounded text-xs bg-gray-600 text-white"
        >
          +{selectedValues.length - 2}
        </span>
      )
    ...
    <div className="flex flex-col">
      ...
      <div className="relative" ref={dropdownRef}>
        ...
        <button
          type="button"
          className={cn(
            "flex h-10 w-full min-w-[180px] items-center justify-between rounded-md border bg-gray-800 px-3 py-2 text-sm transition-colors",
        ...
        >
          ...
          <ChevronDown 
            className={cn(
              "h-4 w-4 transition-transform text-gray-400 flex-shrink-0 ml-2",
              isOpen && "rotate-180"
            )}
          />
        </button>
